...
- **Interface Web Protégée** : Une page web simple avec authentification basique pour tester l'API.
- **Réponse Base64** : L'API renvoie l'audio généré directement encodé en base64 pour une intégration facile sans stockage temporaire sur disque.
...
1. Clonez ce dépôt.
2. Définissez les identifiants de l'interface web dans un fichier `.env` à la racine :
   ```env
   WEB_USERNAME=admin
...
3. Installez les dépendances :
   ...
   ```
4. Définissez les variables d'environnement (optionnel, par défaut `admin`/`password123`) :
   ```bash
